...
public class ImageKitServiceImpl implements ImageKitService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public DocumentationImageResponse uploadDocs(MultipartFile file) throws IOException {
        // Create headers
        HttpHeaders headers = new HttpHeaders();
        headers.set(HttpHeaders.AUTHORIZATION, "Basic " + Base64.getEncoder().encodeToString((PRIVATE_KEY + ":").getBytes()));
        headers.setContentType(MediaType.MULTIPART_FORM_DATA);
        ...
        // Save to table documentation image
        DocumentationImage documentationImage = DocumentationImage.builder()
                .link((String) responseBody.get("url")).build();
        documentationImageRepository.saveAndFlush(documentationImage);
        return DocumentationImageResponse.builder()
                .id(documentationImage.getId())
                .link(documentationImage.getLink())
                .build();
